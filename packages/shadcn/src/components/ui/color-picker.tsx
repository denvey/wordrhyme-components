...
interface Store {
  ...
  /**
   * Sync internal state from a controlled `value` prop without emitting `onValueChange`.
   *
   * In controlled mode, emitting `onValueChange` while applying `value` can create
   * feedback loops (especially in formats like HSL where round-trips can slightly
   * change the serialized string), leading to "Maximum update depth exceeded".
   */
  syncFromValueProp: (value: string) => void;
  setOpen: (value: boolean) => void;
  setFormat: (value: ColorFormat) => void;
  notify: () => void;
}

function areColorsEqual(a: ColorValue, b: ColorValue) {
  return a.r === b.r && a.g === b.g && a.b === b.b && a.a === b.a;
}

function areHsvEqual(a: HSVColorValue, b: HSVColorValue) {
...
function ColorPickerImpl(props: ColorPickerImplProps) {
  ...
  useIsomorphicLayoutEffect(() => {
    if (valueProp !== undefined) {
      store.syncFromValueProp(valueProp);
    }
  }, [valueProp]);
