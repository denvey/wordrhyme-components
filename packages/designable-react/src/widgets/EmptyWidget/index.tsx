...
export const EmptyWidget: React.FC<IEmptyWidgetProps> = observer((props) => {
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="hotkeys-list">
          <div>
            ...
            <IconWidget infer="Shift" /> + Click / <IconWidget infer="Command" /> + A
          </div>
          <div>
            Copy <IconWidget infer="Command" /> + C / Paste <IconWidget infer="Command" />{' '}
            + V
          </div>
          <div>
  ...
  if (!tree?.children?.length) {
    return (
      <div className={prefix}>{props.children ? props.children : renderEmpty()}</div>
    );
  }
  return null;
});
