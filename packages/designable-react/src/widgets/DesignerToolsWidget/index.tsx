...
import { CursorType, ScreenType } from '@wordrhyme/designable-core';
...
const defaultDesignerTools: DesignerToolsType[] = ['HISTORY', 'CURSOR', 'SCREEN_TYPE'];
...
export const DesignerToolsWidget: React.FC<IDesignerToolsWidgetProps> = observer(
  (props) => {
    const tools = props.use ?? defaultDesignerTools;
    ...
    const renderHistoryController = () => {
      if (!tools.includes('HISTORY')) return null;
      return (
    ...
    const renderCursorController = () => {
      ...
      if (!tools.includes('CURSOR')) return null;
      return (
        <Button.Group size="small" style={{ marginRight: 20 }}>
          <Button
            size="small"
    ...
    const renderResponsiveController = () => {
      if (!tools.includes('SCREEN_TYPE')) return null;
