...
import { isStr, isFn, isObj, isPlainObj } from '@wordrhyme/designable-shared';
...
export const IconWidget: React.FC<IIconWidgetProps> & {
  ...
  const takeIcon = (infer: any): React.ReactNode => {
    if (isStr(infer)) {
      ...
      return React.createElement(infer as React.ComponentType<any>, {
        height,
        width,
        fill: 'currentColor',
      });
    } else if (React.isValidElement(infer)) {
      const element = infer as React.ReactElement<any>;
      if (element.type === 'svg') {
        return React.cloneElement(element, {
          height,
          width,
          fill: 'currentColor',
          viewBox: element.props.viewBox || '0 0 1024 1024',
          focusable: 'false',
          'aria-hidden': 'true',
        });
      ...
      if (infer[theme]) {
        ...
        return (
          <IconWidget.ShadowSVG width={width} height={height} content={infer['shadow']} />
        );
      }
      return null;
    }
  ...
  const renderTooltips = (children: React.ReactElement): React.ReactElement => {
    ...
    if (tooltip) {
      const title =
        React.isValidElement(tooltip) || isStr(tooltip) ? tooltip : tooltip.title;
      const props =
