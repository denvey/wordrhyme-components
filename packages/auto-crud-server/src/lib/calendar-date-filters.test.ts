...
function query(
  value: unknown,
  operator: 'eq' | 'ne' | 'lt' | 'lte' | 'gt' | 'gte' | 'isBetween',
  variant: 'date' | 'dateRange' = 'date',
) {
  ...
  return {
    ...
    params: compiled.params.map((value) =>
...
describe('calendar filter serialization without a host date resolver', () => {
  ...
  it.each([
    ...
    'keeps the selected calendar day in %s SQL conditions',
    (operator, boundaries) => {
  ...
  it.each(['eq', 'isBetween'] as const)(
    'uses serialized calendar ranges with %s',
    (operator) => {
      const last = new Date(2026, 8, 16);
      const result = query(
        [serializeCalendarDate(selected), serializeCalendarDate(last)],
        operator,
        'dateRange',
      );
  ...
  it.each(['2026-02-30', '2026-13-01', '2026-00-01', '2026-09-00', '2026-02-29'])(
  ...
  it('preserves legacy numeric timestamp ranges', () => {
    expect(
      query([selected.getTime(), selected.getTime()], 'isBetween', 'dateRange')?.params,
    ).toEqual([start, end]);
  });
});
