...
export function parseAsStringEnum<T extends string>(validValues: readonly T[]): Parser<T | null> & {
...
export function useUrlState<T>(
  ...
  const optionsRef = React.useRef({ key, parser, defaultValue, history, clearOnDefault, scroll });
  ...
  const setValue = React.useCallback(
    (updater: T | ((prev: T) => T)) => {
      setValueInternal((prev) => {
        ...
        queueMicrotask(() => {
          const { key, parser, defaultValue, history, clearOnDefault, scroll } = optionsRef.current;
          const params = getUrlParams();
          const serialized = parser.serialize(newValue);
          const defaultSerialized = defaultValue !== undefined
            ? parser.serialize(defaultValue)
            : "";
