...
import { TreeNode, createBehavior, createResource } from '@wordrhyme/designable-core';
import {
  useTreeNode,
  TreeNodeWidget,
  DroppableWidget,
    ...
    return (
      <ArrayBase disabled>
        ...
        >
          {columns.map((node) => {
            ...
            return (
              <Table.Column
                ...
                dataIndex={node.id}
                title={
                  <div data-content-editable="x-component-props.title">{props.title}</div>
                }
          ...
          {columns.length === 0 && <Table.Column cell={() => <DroppableWidget />} />}
        </Table>
        {additions.map((child) => {
          return <TreeNodeWidget node={child} key={child.id} />;
        })}
